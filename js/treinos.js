...
(() => {
  ...
  function montarItemTreino(exercicioId, series, repMin, repMax, carga, intervalo, unidadeRep, unidadeCarga) {
    ...
    return {
      ...
      carga: carga ?? null,
      // carga de aquecimento: a série que prepara o movimento, registrada
      // separada para não se misturar com a carga de trabalho na progressão
      cargaAquecimento: null,
  ...
  function cargaParada(exercicioId, exceto) {
    ...
    const atual = vals[0];
    // parado é ficar na mesma carga *sem* ganhar repetição: subir de 8 para 12
    // reps com o mesmo peso é progressão, e aí não cabe cutucar
    let n = 0;
    for (const v of vals) {
      if (v.carga !== atual.carga) break;
      ...
      .map((it, i) => {
        ...
        <div class="exec-item${it.feito ? ' feito' : ''}" data-i="${i}">
          ...
          <div class="exec-campos">
            ${(it.unidadeCarga || 'kg') === 'kg' ? `<div class="field">
              <label>Aquecimento (kg)</label>
              <input type="number" class="ex-aquec" min="0" step="0.5" value="${it.aquecimento ?? ''}" inputmode="decimal" placeholder="—" />
            </div>` : ''}
            <div class="field">
  ...
  async function finalizarTreino() {
    ...
    if (treino) {
      ...
      for (const ex of treino.exercicios || []) {
        ...
        // o aquecimento do dia também vira o padrão da próxima vez
        const aquec = itens.find((i) => i.exercicioId === ex.exercicioId && i.feito && i.aquecimento != null);
        if (aquec && aquec.aquecimento !== ex.cargaAquecimento) {
          ex.cargaAquecimento = aquec.aquecimento;
          mudou = true;
        }
      }
    ...
    wrap.innerHTML = emEdicao.exercicios
      ...
      <div class="mt-ex" data-i="${i}">
        ...
        <div class="mt-ex-campos">
          ...
          <div class="field"><label>Aquecimento</label><input type="number" class="c-aquec" min="0" step="${(e.unidadeCarga || 'kg') === 'kg' ? '0.5' : '1'}" value="${e.cargaAquecimento ?? ''}" placeholder="—" /></div>
  ...
  function lerEditor() {
    $$('#mt-exercicios .mt-ex').forEach((el) => {
      ...
      const campoAquec = $('.c-aquec', el);
      if (campoAquec) ex.cargaAquecimento = campoAquec.value === '' ? null : Number(campoAquec.value);
  ...
  function renderEvolucao() {
    ...
    // progrediu = ganhou força estimada (carga e repetições juntas); sem reps
    ...
    $('#evo-resumo').innerHTML = `
      <div class="tiles">
        ...
        <div class="tile"><div class="t-label">Com evolução</div><div class="t-value">${evoluiram}</div><div class="t-sub">força estimada maior que a primeira</div></div>
      </div>`;
    ...
    wrap.innerHTML = blocoDuracao + [...porExercicio.entries()]
      .map(([id, e]) => {
        ...
        const resumoAquec = comAquec.length
          ? `Aquecimento ${
              comAquec.length > 1 && comAquec[0].aquecimento !== comAquec[comAquec.length - 1].aquecimento
                ? `${fmt(comAquec[0].aquecimento)} → ${fmt(comAquec[comAquec.length - 1].aquecimento)} kg`
    ...
    for (const [id, e] of porExercicio.entries()) {
      ...
      if (escalas.y && valoresY.length && (e.forca || e.temAquecimento)) {
        const min = Math.min(...valoresY);
        const max = Math.max(...valoresY);
        const folga = Math.max((max - min) * 0.12, max * 0.04, 0.5);
        escalas.y = {
          ...escalas.y,
      ...
      // Força estimada: a linha tracejada que junta carga e repetições. É ela
      // que mostra progressão quando o peso sobe e a repetição cai.
      if (e.forca) {
        linhas.push({
          type: 'line',
          ...
          backgroundColor: cssVar('--alvo-line'),
          borderWidth: 2,
          borderDash: [6, 4],
          pointRadius: 2,
          pointHoverRadius: 5,
      ...
      // Aquecimento: fica discreto, embaixo — é registro, não progressão
      if (e.temAquecimento) {
        linhas.push({
          type: 'line',
          label: 'Aquecimento (kg)',
          data: e.pontos.map((p) => p.aquecimento),
          yAxisID: 'y',
          sufixoRotulo: un,
          semRotulo: true,
          borderColor: corComAlfa(cssVar('--muted'), 0.8),
          backgroundColor: corComAlfa(cssVar('--muted'), 0.8),
          borderWidth: 1.5,
          pointRadius: 2,
          pointHoverRadius: 5,
          tension: 0,
  ...
  async function init() {
    ...
    $('#exec-itens').addEventListener('input', (ev) => {
      ...
      if (ev.target.classList.contains('ex-aquec')) it.aquecimento = ev.target.value === '' ? null : Number(ev.target.value);
