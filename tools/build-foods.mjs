...
import { SORVETES } from './sorvetes.mjs';
...
async function main() {
  ...
  // 4b) Chocolates e bombons (marcas de massa, gourmet e bean-to-bar)
  let nChocolates = 0;
  for (const it of CHOCOLATES) {
    if (push({
      i: stableId('m', it.n), n: it.n, f: 'm',
      kcal: round1(it.kcal), p: round1(it.p), c: round1(it.c), g: round1(it.g),
      m: (it.m || []).map(([l, gr]) => [l, gr]),
  ...
  // 4d) Sorvetes, picolés e gelaterias
  let nSorvetes = 0;
  for (const it of SORVETES) {
    if (push({
      i: stableId('m', it.n), n: it.n, f: 'm',
      kcal: round1(it.kcal), p: round1(it.p), c: round1(it.c), g: round1(it.g),
      m: (it.m || []).map(([l, gr]) => [l, gr]),
  ...
  const corpo = JSON.stringify({ v: 59, foods });
  ...
  writeFileSync(MANIFESTO, JSON.stringify({ v: 59, h: hash, n: foods.length }));
